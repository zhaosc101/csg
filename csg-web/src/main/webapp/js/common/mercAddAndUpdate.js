jQuery(function($) {
	...
	if($("#fromQueryPage").val()==undefined){
		...
		on('success.form.fv', function(e) {
			...
			e.preventDefault();
			var $form = $(e.target);
			var url = '/obm/onlineBusinessQuery/submitNext';
	...
	if($("#olePayFlg").val()=='0'){
		...
		if ($("#singleLimit").val() != '' && !$("#noSingleLimit").is(":checked")) {
			...
			if(parseFloat($("#singleLimit").val())<1||parseFloat($("#singleLimit").val())>9999999999){
				Modal.alert({
					msg : '单笔最高限额必须是大于1小于9999999999的数字'
				}).on(function(e) {
					enableApplyButton();
				});
				return false;
		...
		if($("#movePay").is(":checked")){
			if($("#movePayRate").val()==''){
				...
				var reg = /^(?!(\.0{1,2})?$)(\d(\.\d{1,2})?|10|10.00|10.0)$/;
				if(!reg.test($("#movePayRate").val())){
					Modal.alert({
						msg : '移动支付方式费率为0到10的数字且小数点后最多保留两位有效数字'
					}).on(function(e) {
						enableApplyButton();
					});
					return false;
...
function atleastChoicePayType() {
	var ob1 = $("input[id='eBankTypePersonal']");
...
function changeOlePay(val){
	if(val=='0'){
		$("#olePayDiv").show();
		$("#noSingleLimit").attr("checked",false);
		$("#singleLimit").removeAttr("readonly");
		$("#noDayLimit").attr("checked",false);
		$("#dayLimit").removeAttr("readonly");
		
	}else{
		/*if(!$("#noSingleLimit").is(':checked')){
			if($("#singleLimit").val()==""||null==$("#singleLimit").val()){
				Modal.alert({
					msg : '单笔最高限额不能为空'
				});
				return;
		...
		if(!$("#noDayLimit").is(':checked')){
			if($("#dayLimit").val()==""||null==$("#dayLimit").val()){
				Modal.alert({
					msg : '每日最高限额不能为空'
				});
				return;
