 ...
 */
	...
	$.jstree.core.prototype = {
		 ...
		 * @name jstree-wholerow-clicked([full])
		...
		get_data : function (obj) {
			obj = this.get_node(obj);
			return (!obj || obj.id === $.jstree.root) ? false : obj.data;
		},
		/**
			...
			for(i = 0, j = p.length; i < j; i++) {
				if(!m[p[i]].state[ t ? 'selected' : 'checked' ]) {
					...
					if(s && s.length) {
						...
						s.children('.jstree-anchor').addClass('jstree-clicked');
						s.children('.jstree-wholerow').addClass('jstree-wholerow-clicked');
						this._data[ t ? 'core' : 'checkbox' ].selected.push(s.attr('id'));
					}
				}
			}
			this._data[ t ? 'core' : 'checkbox' ].selected = $.vakata.array_unique(this._data[ t ? 'core' : 'checkbox' ].selected);
 ...
 */
	 ...
	 * @plugin sort
	 */
	$.jstree.defaults.sortbak = function (a, b) {
		//return this.get_type(a) === this.get_type(b) ? (this.get_text(a) > this.get_text(b) ? 1 : -1) : this.get_type(a) >= this.get_type(b);
		return this.get_text(a) > this.get_text(b) ? 1 : -1;
	};
	$.jstree.defaults.sort = function (a, b) {
		//return this.get_type(a) === this.get_type(b) ? (this.get_text(a) > this.get_text(b) ? 1 : -1) : this.get_type(a) >= this.get_type(b);
		return this.get_data(a) > this.get_data(b) ? 1 : -1;
	};
